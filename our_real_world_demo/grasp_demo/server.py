...
class GraspModel():
    ...
    def grasp_object(self,category):
        ...
        while flag is True:
            ...
            for idx, phrase in enumerate(phrases):
                if category in phrase:
                    ...
                    detect_result.data = [bx,by,tx,ty]
        ...
        while grasppoint is None:
            time.sleep(0.1)
        grasp_rot = np.array(grasppoint[:9])
        grasp_rot = grasp_rot.reshape(3,3)
        
        trans = grasppoint[9:]
        grasp_point_pose = np.ones((4,4))
        grasp_point_pose[:3,:3] = grasp_rot
        grasp_point_pose[:3,3] = trans

        # grasp point in graspnet is diffreent axis
        ...
        new_grasp_rot = grasp_rot.dot(T_1)
        # new_grasp_rot = T_1.dot(grasp_rot)
        grasp_point_pose[:3,:3] = new_grasp_rot
        
        grasp_point_pose_in_base = trans_pose_to_base(grasp_point_pose, ee_in_base_mat)
        
        grasp_quat = mat2quat(grasp_point_pose_in_base[:3,:3])
        print("delta rot is:",grasp_quat)
        
        c_point_in_base = grasp_point_pose_in_base[:,3]
        # c_point_in_base = trans_point_to_base(c_point_trans,ee_in_base_mat)
        # print("the detected grasp point is:",c_point_in_base)
        
        self.reset_pose(self.reset_top_down_positions,grasp=False)


        ee_top_in_base_mat = self.controller.last_eef_pos
        delta_position = c_point_in_base[:3] - ee_top_in_base_mat[:3,3] + [0,0,-0.2]
